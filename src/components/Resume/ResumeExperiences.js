...
const ResumeExperiences = (props) => {
  ...
  if (props.position === "right") {
    textClasses = " lg:text-left pr-8 mr-auto lg:ml-5";
  ...
  return (
    <Fragment>
      <div className="w-full lg:w-1/2 px-4">
        ...
        >
          <span className={`bg-black border-white rounded-full border-4 w-4 h-4 top-1 left-0 absolute ${circleClasses}`}></span>
          <h3 className="text-neutral-100 font-bold text-xl mb-1">
            {props.jobDescription}
          </h3>
          <p className="text-neutral-300 font-semibold mb-3 text-base leading-6">
            {props.companyName}
          </p>
          <p className="bg-black-300 px-4 py-1 font-bold text-sm leading-6 bg-neutral-100 rounded-full inline-flex mb-5">
            {props.from} - {props.to}
          </p>
        </div>
      </div>
